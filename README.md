...
_Note that the `Foundation` library just got split up into the three libraries `Foundation`, `FoundationNetwork`, and F`oundationXML`, and on Windows there is currently the bug [SR-14578](https://bugs.swift.org/browse/SR-14578), so the project currently does _not_ run on Windows. This will hopefully get resolved soon._
